...
object RouteBuilder {
  ...
  fun onInput(event: KeyInputEvent) {
    ...
    if (Keyboard.isKeyDown(Keyboard.KEY_NUMPADENTER)) {
      ...
      val selectedBlock = (this.route.lastOrNull()?.block ?: player.getStandingOnFloor().up()).down()
      when (tempBlock) {
        null -> {
    ...
    RenderUtil.filledBox.remove(tempBlock!!)
    this.tempBlock = this.getOffset(tempBlock!!)
    RenderUtil.filledBox.add(tempBlock!!)
  }
  ...
  private fun handleKey() {
    for (key in 0 until Keyboard.KEYBOARD_SIZE) {
      this.keys[key] = Keyboard.isKeyDown(key)
    }
  }
